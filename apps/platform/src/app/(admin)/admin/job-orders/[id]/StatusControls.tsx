...
const OPTIONS: { value: "open" | "closed" | "filled" | "cancelled"; label: string }[] = [
  { value: "open", label: "Lagi buka" },
  { value: "filled", label: "Penuh" },
...
export default function StatusControls({
  ...
  id: string;
  current: "open" | "closed" | "filled" | "cancelled";
}) {
  const [pending, start] = useTransition();
  const [value, setValue] = useState(current);
  const [error, setError] = useState<string | null>(null);

  function change(next: typeof current) {
    ...
    start(async () => {
      try {
        await updateJobOrderStatus(id, next);
      } catch (err) {
        setValue(prev);
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {pending && (
        <div className="text-[11px] text-pg-ink-500 mt-1">Menyimpan…</div>
      )}
      {error && (
