...
Built from the Claude Design deliverables in `~/Downloads/Perantau Global Design System/akademi-redesign/` (Fase-1 Flow-IA + Fase-2 Hifi-Mobile). Autonomous overnight build. **Core idea:** the course is a *paspor* — each module completed = a *cap/stempel*; the certificate is visible from day one; one idea per screen.

**What shipped (PR — feat/akademi-xendit-payment):**
- **Chunked reading player** (`components/pg/academy/LessonPlayer.tsx`): blocks → tap-next cards (`chunkBlocks()` auto-splits by heading, or explicit `chunk` markers), per-card illustration/audio slot, "Pelajaran X / N" spatial anchor, in-lesson WhatsApp help. **Degrades gracefully** for un-chunked content → safe for the live free finansial course.
- **Quiz**: one question per screen + **secure per-option rationale on fail** — sourced from admin-only `academy_lesson_keys.keys[qid].rationale` via the `grade_academy_quiz` RPC, which now returns `feedback` echoing ONLY the learner's own wrong picks (never the correct answer).
- **New screens/routes**: `/akademi/[slug]/modul/[n]` (Module Intro) + `/akademi/[slug]/modul/[n]/selesai` (Cap Modul ceremony).
- **PassportProgress** component (module stamps + dual cert path), Beranda resume card (`AkademiResumeCard`), Akademi list "Kelas saya" resume, program-detail "cover paspor" + certificate terminal (dual path kursus+psikotes → 2 sertifikat).
- **Gating**: the passport treatment (stamps, dual-path, cap ceremony, 2-cert terminal) is **Paspor-only** (`program.category === 'paspor'`). Other courses keep plain progress + single cert. Psikotes leg reuses existing `academy_enrollments.external_status`.

**DB migrations (additive):**
- **0085 APPLIED to prod**: `academy_modules.content` (module intro outcomes/est_minutes) + `grade_academy_quiz` rationale feedback. Verified live + advisors clean (no new issues).
- **0084 APPLIED to prod**: Akademi payment plumbing (enrollment `payment_status` + `set_..._pending` owner RPC + `mark_..._paid` service-role RPC).
- **0082 NOT applied** (the reworked Paspor Arab seed, 24 lessons). Generated by `scripts/gen-academy-seed.py` from `docs/akademi-perantau/paspor-arab-siap-kerja.json`. Idempotent (ON CONFLICT) + deterministic uuid5. **Stays draft until payment is live.**

**Materials rework**: 24 lessons reworked by 6 parallel agents — chunk-friendly cards (82 cards, every one with a `media.alt` illustration cue), heading `anchor`s, module `outcomes`, 68 per-option quiz rationales. **All facts/numbers/contacts preserved.** Source backed up at `docs/akademi-perantau/_rework/_original-course.json`.

**Verification**: typecheck ✅ lint ✅ prod build ✅ (all routes incl. new ones) · DB schema/RPC verified via SQL · pure-logic sanity (chunking/caps) ✅ · security advisors clean.

**Still GATED on Panji / payment launch (NOT done tonight, by design):**
1. Apply **0082** seed (publish Paspor course) when ready to launch.
2. Xendit account + secrets (`XENDIT_SECRET_KEY`, `XENDIT_WEBHOOK_TOKEN`) in Vercel platform env + Supabase Edge secret.
3. Deploy `supabase/functions/xendit-webhook` (`verify_jwt=false`) + set webhook URL in Xendit.
4. Then flip Paspor `status='published'` + end-to-end test in Xendit Test Mode.

**Also produced**: `docs/akademi-perantau/BRIEF-REDESIGN-PORTAL-AKADEMI.md` (the brief sent to Claude Design).


## 2026-06-18 — Welder Mexico + ads boost + journey fixes + Vokasi UI event email ✅ SHIPPED

Four threads, all shipped/live this session.

**1. Welder Mexico content (`welder-heavy-steel-plate-fabrication`)** — filled compensation in `positions.content` (DB data op via MCP, live via ISR): `cardMeta.salary` "18.240 – 18.430 MXN" + `salaryNote` "per bulan" + top-level `content.salaryIdr` "Rp19.000.000 / bulan" + `fee.amount` "Rp20.000.000". Pattern for clean salary: keep the number unit-free in `cardMeta.salary`; the period label comes from the card template / `salaryNote`. (Positions whose salary has an inline unit caused the double-"/bulan" bug — see #3.)

**2. CLAUDE.md refresh (PR #168, merged):** phase-status was frozen at 2026-05-24; rewrote it with a curated "Major milestones since launch" list + added the doc-maintenance convention at the top (CLAUDE.md = architecture, TASKS.md = handoff, memory = per-fact; `/session-wrap` refreshes phase-status on architectural change).

**3. Ads boost (Meta CLI, no deploy):** 5 campaigns ACTIVE — barista + head-driller @Rp150K/day, konstruksi + perawat + spa @Rp50K/day. Welder + Espresso HELD (no poster yet). New Vokasi UI **event** campaign `PG_event_work-travel-repeat_2606_v1` @Rp100K/day (CompleteRegistration optimization, auto-stop at event start). Full detail in `~/Cowork/advertising/decisions.md`. **Operating principle (Panji): with Andromeda + Advantage+, don't set gender/age manually — country+budget+activate, QA the creative.** (memory `andromeda-skip-manual-audience`).

**4. Signup-journey audit + fixes (PR #169, merged + verified E2E):** full agent-email signup walkthrough found 4 bugs, all fixed:
- **auth-callback** was a server route handler but signUp-confirm returns the session in the URL **hash** (implicit flow) → every new candidate bounced to `/auth/sign-in?error=missing_code` unauthenticated. Now a **client page** (`apps/platform/src/app/auth/callback/page.tsx`) consumes the hash + sets cookie session; side-effects (CompleteRegistration CAPI + cv-materialize + fbp/fbc) moved to `POST /api/auth/post-confirm`. **Do not revert to a route handler** (memory `auth-callback-implicit-flow`).
- **CSP** on platform blocked Meta's CAPI param-builder → added `capi-automation.s3.us-east-2.amazonaws.com` to `script-src` in `apps/platform/next.config.ts`.
- **Double "/bulan"** on dashboard explore cards → `dashboard/page.tsx` now only appends "/bulan" when `salary` has no "/".
- **"Talent Hub"** in the Supabase Auth "Confirm signup" email → changed to "Perantau Global" (dashboard template, done via browser). NOTE: `apps/web/src/messages/id.json` still has a separate **"Global Talent Hub"** program section (own LP, GTR cert, KLOB) — left untouched (likely a distinct program; confirm with Panji before editing).

**5. Vokasi UI event thank-you email (PR #170, merged + verified E2E):** new `apps/web/src/lib/email.ts` (zero-dep Resend sender + `buildEventThankYouEmail`) wired into `/api/event/[slug]` (fires after fresh insert, says Zoom link sent H-1). **Gotcha: `RESEND_API_KEY` was NOT in the `perantauglobal-web` Vercel runtime env** (only in Supabase SMTP config) — added it (Sensitive, Prod+Preview) + redeployed. Sensitive vars are write-only (can't `vercel env pull` the value). Backfilled the thank-you to 11 pre-existing registrants via `scripts/backfill-event-thankyou-wtr.mjs` (Panji ran it with his own key; the script is uncommitted — has PII inline).

**6. Ad assets + full hiring launch (later same day):**
- Generated the missing **Espresso LP hero** via fal-ai Seedream v4.5 (only active position without one; recipe `hero-image-recipe`), PR #171. Cleaned Espresso `content` cosmetics (salary/age/typo, DB op).
- Saved 3 final posters (Panji) → `~/Documents/Perantau Global Content Assets/PG_{MX,SA,JP}_*_2606.png`.
- **Hiring budget capped Rp500K/day** (Panji): **P1 @80K** barista/head-driller/welder-MX(new)/espresso(new), **P2 @45K** konstruksi/perawat/spa/manufaktur(reactivated + new-poster creatives). Welder MX `120249820527890392` + Espresso `120249820412530392` = NEW campaigns. Event WTR raised 100K→200K (≈Rp1jt to Tue; event budget is SEPARATE from the 500K cap). Account total ≈Rp700K/day. All ACTIVE, smoke-tested (Lead→pending+CAPI OK).
- Synced espresso hero to `position-media` bucket (edge fn `seed-position-media` v4 + slug added, 31 uploaded, PR #172) → portal explore card fixed.

**Open / next:**
- **Auto Zoom-link reminder H-1** for events (cron + Resend to all registrants) — offered, declined for now. Zoom link sent manually H-1.
- New ad creatives (espresso/welder/manufaktur) were PENDING_REVIEW at activation — confirm they exited review + are delivering.
- `apps/web/src/messages/id.json` still has a separate **"Global Talent Hub"** program section — left untouched; confirm with Panji whether to retire/rename.
- `scripts/backfill-event-thankyou-wtr.mjs` is uncommitted with PII inline — delete when no longer needed.
