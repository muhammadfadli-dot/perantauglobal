// grade-cv — AI CV grader.
//
// Batch 1 (extraction): read a CV from private storage -> Gemini Flash-Lite ->
//   structured JSON + completeness, computing umur/pengalaman in code.
// Batch 2 (fit): score a parsed CV against ONE position's requirements with
//   Gemini Flash (text-only), and flag CV/answer contradictions for admin.
//
// Modes (POST JSON body):
//   { document_id }                  grade one CV (extraction)
//   { backfill, limit }              extract up to N ungraded CVs
//   { application_id }               score one application's CV-to-position fit
//   { fit_backfill, limit }          score up to N applications missing fit
//
// Auth (verify_jwt=true): service_role/admin may do anything; an authenticated
// candidate may extract only their OWN CV. Fit modes are privileged-only.
//
// Secrets: SUPABASE_URL + SUPABASE_SERVICE_ROLE_KEY (auto) + AI_GATEWAY_API_KEY.
...
async function persistDocError(doc: DocRow, status: string, error: string) {
  await svc.from("cv_assessments").upsert({
    candidate_id: doc.candidate_id, document_id: doc.id, status, error,
    model: EXTRACT_MODEL, prompt_version: PROMPT_VERSION, updated_at: new Date().toISOString(),
...
async function gradeOne(doc: DocRow): Promise<{ ok: boolean; status: string; error?: string }> {
  ...
  try {
    ({ res, payload } = await callGateway(EXTRACT_MODEL, [
  ...
  const { error } = await svc.from("cv_assessments").upsert({
    candidate_id: doc.candidate_id, document_id: doc.id, parsed, derived,
    quality: { skor_kelengkapan: qualityScore, kekurangan: kualitas.kekurangan ?? [] },
    quality_score: qualityScore, status: "ok", error: null,
    model: EXTRACT_MODEL, prompt_version: PROMPT_VERSION,
    tokens_in: usage.prompt_tokens ?? null, tokens_out: usage.completion_tokens ?? null, cost_usd: cost,
...
async function positionFields(slug: string): Promise<FieldRow[]> {
  ...
  const { data } = await svc.from("position_application_fields")
    .select("field_key, field_label, field_type, importance, options")
    .eq("position_slug", slug).order("sort_order", { ascending: true });
...
async function persistFitError(app: AppRow, status: string, error: string) {
  await svc.from("application_cv_fit").upsert({
    application_id: app.id, position_slug: app.position_slug, status, model: FIT_MODEL,
    prompt_version: FIT_PROMPT_VERSION, reasons: { error }, updated_at: new Date().toISOString(),
  }, { onConflict: "application_id" });
  return { ok: false, status, error };
}

async function scoreFit(app: AppRow): Promise<{ ok: boolean; status: string; error?: string }> {
  // Need a successfully-parsed CV for this candidate.
  const { data: asmt } = await svc.from("cv_assessments")
    .select("id, parsed").eq("candidate_id", app.candidate_id).eq("status", "ok")
    .order("created_at", { ascending: false }).limit(1).maybeSingle();
  if (!asmt) {
    await svc.from("application_cv_fit").upsert({
      application_id: app.id, position_slug: app.position_slug, assessment_id: null,
      fit_score: null, status: "skipped", reasons: {}, verification: [], has_flags: false,
      model: FIT_MODEL, prompt_version: FIT_PROMPT_VERSION, updated_at: new Date().toISOString(),
    }, { onConflict: "application_id" });
  ...
  const fieldsText = fields.length
    ? fields.map((f) => `- [${f.importance}] ${f.field_label}`).join("\n")
  ...
  const answersText = fields
    ...
    .map((f) => `- ${f.field_label}: ${renderAnswer(f, answers[f.field_key])}`)
    .join("\n");
  ...
  let res: Response, payload: Record<string, unknown> | null;
  try {
    ({ res, payload } = await callGateway(FIT_MODEL, [{ type: "text", text: prompt }], FIT_JSON_SCHEMA));
  } catch (e) { return await persistFitError(app, "error", `gateway fetch: ${String(e).slice(0, 200)}`); }
  ...
  const { error } = await svc.from("application_cv_fit").upsert({
    application_id: app.id, assessment_id: asmt.id, position_slug: app.position_slug,
    fit_score: fitScore, reasons: { alasan: out.alasan ?? "", yang_kurang: out.yang_kurang ?? [] },
    verification, has_flags: hasFlags, status: "ok",
    model: FIT_MODEL, prompt_version: FIT_PROMPT_VERSION, cost_usd: usage.cost ?? null,
...
Deno.serve(async (req) => {
  ...
  if (body.fit_backfill) {
    ...
    const { data: apps, error } = await svc.from("applications")
      .select("id, candidate_id, position_slug, answers, application_cv_fit(status)")
      .order("created_at", { ascending: true }).limit(800);
    if (error) return json({ error: error.message }, 500);
    const pending = (apps ?? []).filter((a: Record<string, unknown>) => {
      const f = a.application_cv_fit as Array<{ status: string }> | null;
      return !f?.length || f[0].status === "error";
    });
  ...
  if (body.application_id) {
    ...
    const { data: app, error } = await svc.from("applications")
      .select("id, candidate_id, position_slug, answers").eq("id", body.application_id).single();
  ...
  if (body.backfill) {
    ...
    const { data: docs, error } = await svc.from("candidate_documents")
      .select("id, candidate_id, file_path, mime_type, cv_assessments(status)")
      .eq("doc_type", "cv").order("uploaded_at", { ascending: true }).limit(500);
    if (error) return json({ error: error.message }, 500);
    const pending = (docs ?? []).filter((d: Record<string, unknown>) => {
      const a = d.cv_assessments as Array<{ status: string }> | null;
      return !a?.length || a[0].status === "error";
    });
  ...
  const { data: doc, error } = await svc.from("candidate_documents")
    .select("id, candidate_id, file_path, mime_type, doc_type").eq("id", documentId).single();
