...
const BTN_BASE =
  "inline-flex items-center justify-center gap-2 font-semibold text-base rounded-xl border-0 cursor-pointer no-underline transition-transform duration-75 active:scale-[0.985] tracking-tight";
...
function buttonClasses({ variant = "primary", block, small, className }: ButtonProps) {
  return [
    BTN_BASE,
...
export function Button(
  ...
  return (
    <button {...rest} className={buttonClasses({ variant, block, small, className, children })}>
      {children}
    </button>
  );
}
...
export function ButtonLink(
  ...
  return (
    <Link href={href} {...rest} className={buttonClasses({ variant, block, small, className, children })}>
      {children}
    </Link>
  );
}
...
export function Chip({
  ...
  return (
    <span
      className={[
        "inline-flex items-center gap-1.5 h-9 px-3.5 text-sm font-semibold rounded-full whitespace-nowrap border-[1.5px] transition-colors",
        active
          ? "bg-pg-ink-900 text-white border-pg-ink-900"
          : "bg-pg-white text-pg-ink-700 border-pg-ink-200",
        className ?? "",
      ].join(" ")}
    >
      {children}
...
export function Eyebrow({ children, tone = "red" }: { children: React.ReactNode; tone?: "red" | "ink" | "ok" }) {
  const color =
