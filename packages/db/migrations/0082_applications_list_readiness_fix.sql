-- 0082_applications_list_readiness_fix.sql
--
-- Fix the admin applications list (/admin/applications). Two columns were broken:
--
--   1. READINESS — dead since migration 0038. The list RPC ships `per_field: {}`
--      (0038 dropped per-field detail from the list "since the list view doesn't
--      render it"), but ReadinessBadge used an empty per_field as its
--      "no requirements → show —" guard. So the guard ALWAYS tripped and every
--      one of the ~717 pool rows rendered "—", even though hard_pass/score_pct
--      were right there in the payload.
--
--      New semantics: the list surfaces a QUALIFYING-gate signal. We add
--      `has_gate` = does this position have a REQUIRED field whose options carry
--      a `qualifying` flag? The badge then shows "Lolos syarat" / "Belum lolos"
--      ONLY where a real gate exists, and "—" where there is none (instead of a
--      misleading presence-% colored by qualifying — e.g. "100%" amber). This
--      matches the completeness-vs-qualifying split (migration 0077): the admin
--      list keys off qualifying (hard_pass), candidate surfaces key off presence.
--
--   2. FIT CV — conflated "no CV" with "never graded". "Belum CV" was derived
--      from grader status='skipped', but ~133 pool apps were never graded at all
--      (status NULL) so they showed an ambiguous "—" regardless of whether the
--      candidate had a CV. We add `candidate_has_cv` so the cell can show
--      "Belum CV" for ANY candidate without a CV doc, and distinguish
--      "Belum dinilai" (has CV, not yet graded) from "Gagal" (error).
--
-- Same body as 0073 + `has_gate` in the readiness jsonb + a `candidate_has_cv`
-- column. DROP first because the return type changes. SECURITY INVOKER (default):
-- RLS on the underlying tables/views still applies to the caller.
  ...
  WITH filtered AS (
    SELECT
      ...
      jsonb_build_object(
        ...
        END,
        -- Does this position define a real qualifying gate? (a required field
        -- whose options carry a `qualifying` flag). Only then is hard_pass a
        -- meaningful "lolos/belum lolos" signal; otherwise the badge shows "—".
        'has_gate', EXISTS (
          SELECT 1 FROM position_application_fields pgf
          WHERE pgf.position_slug = a.position_slug
            AND pgf.importance = 'required'
            AND pgf.options IS NOT NULL
            AND EXISTS (
              SELECT 1 FROM jsonb_array_elements(pgf.options) o WHERE o ? 'qualifying'
            )
        )
      ) AS readiness,
      ...
      -- Does the candidate have ANY CV document? Drives "Belum CV" regardless of
      -- whether the grader has run yet (fixes the "—" vs "Belum CV" inconsistency).
      EXISTS (
