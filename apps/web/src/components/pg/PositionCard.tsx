...
export function PositionCard({ p }: { p: Position }) {
  return (
    ...
    >
      <div
        className="relative px-4 pt-4 pb-4 min-h-[108px] text-white"
        style={{
          background:
            "radial-gradient(ellipse at 80% 10%, var(--pg-overlay-white-strong), transparent 60%), var(--pg-red-600)",
        }}
      >
        <div className="absolute top-3 right-3 w-9 h-9 rounded-[10px] grid place-items-center text-white" style={{ background: "var(--pg-overlay-white-med)" }}>
          <Icon name={p.icon} size={20} stroke={2} />
        </div>
        <div className="text-[11px] font-bold tracking-[0.14em] uppercase opacity-85">{p.country}</div>
        <div className="text-2xl font-extrabold leading-tight tracking-tight mt-2">{p.role}</div>
      </div>
      <div className="p-4">
